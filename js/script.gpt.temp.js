javascript: (() => {
  try {
    const R = window.__CVZ_RESUME__ = window.__CVZ_RESUME__ || {};
    const KEY = "__cvz_export_state_v1__";
    const VER = "cvz-bookmarklet-1.1";
    const now = () => Date.now();
    ...
    const Store = {
      ...
      save(st) {
        try {
          ...
          console.warn("convoviz: failed to save state", e);
        }
      },
      reset() {
      ...
      async fetchBlob(url, opts) {
        ...
        while (true) {
          ...
          if (resp.status === 429) {
            ...
            saveDebounce(true);
            if (UI) UI.setStatus("Rate limited (429) → sleeping " + fmtMs(waitMs));
            addLog("HTTP 429 (blob). Sleeping " + fmtMs(waitMs) + " then retrying…");
            await sleep(waitMs, signal);
            continue;
          }
          if (resp.ok) {
          ...
          if ([502, 503, 504].includes(resp.status) && attempt < 3) {
            attempt++;
            const w = 1000 * attempt;
            addLog("HTTP " + resp.status + " (blob) retrying in " + fmtMs(w) + "…");
            await sleep(w, signal);
            continue;
          }
          throw new Error("HTTP " + resp.status + " " + (txt ? txt.slice(0, 200) : ""));
    ...
    const UIImpl = {
      ...
      inject() {
        ...
        d.querySelector("#cvz-reset").onclick = () => {
